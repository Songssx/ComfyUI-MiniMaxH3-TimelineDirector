...
选区内的重叠区使用原生Guide固定在生成视频的对应帧位置；同一素材位于选区外的部分作为普通 `<Video N>` 上下文参考。
...
选区内的整段视频会按时间线位置固定。H3多帧Guide只接受 `5 + 17n` 帧，因此导演台会链式组合 `5、22、39…` 帧的合法批次与单帧Guide，不丢弃尾部帧。约1秒的24帧重叠区会以 `22 + 1 + 1` 完整固定24帧。
...
每个选区重叠都会按目标帧位置建立Guide。只有跨越选区边缘的区外上下文会拆分成最长15秒的普通视频参考，最多3个。
