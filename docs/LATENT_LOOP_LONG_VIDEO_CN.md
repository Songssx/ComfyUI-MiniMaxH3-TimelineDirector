...
从第二段开始，`MiniMax H3 Latent 循环续段`还会把上一段尾部 latent 写入本段开头，
并按实际 `overlap_frames` 生成时间噪声遮罩。遮罩在重叠区对应的视频 latent token 上
从 `0` 线性增加到 `1`：开头尽量保留上一段 latent，越接近重叠区末端允许越多去噪，
重叠区之后恒为 `1`、完全正常生成。开启音频 latent 延续时，音频遮罩也按对应 token
数量执行同样的线性过渡。
